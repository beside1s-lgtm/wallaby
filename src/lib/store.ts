...
export const initializeData = (school: string) => {
  getLocalStorage(getKey(school, 'students'), initialStudents.filter(s => s.school === school));
  getLocalStorage(getKey(school, 'measurementItems'), initialItems);
  getLocalStorage(getKey(school, 'records'), initialRecords.filter(r => r.school === school));
};
...
// Measurement Items
export const getItems = (school: string): MeasurementItem[] => getLocalStorage(getKey(school, 'measurementItems'), initialItems);
