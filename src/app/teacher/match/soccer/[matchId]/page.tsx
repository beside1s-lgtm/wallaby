...
type HalfStats = Record<string, PlayerStats>;
type MatchStats = Record<string, HalfStats>;
...
export default function SoccerMatchPage() {
  ...
  const handleSave = async () => {
    ...
    const categoryToItemName: Record<StatCategory, string> = {
      goal: '득점', pass: '패스', dribble: '돌파성공', defense: '수비성공',
    };
    
    const categoryToScoreMultiplier: Record<StatCategory, number> = {
      goal: 3, pass: 1, dribble: 2, defense: 1,
    };
    ...
    try {
      ...
      toast({ title: '기록 저장 완료', description: `${selectedHalf === '1' ? '전반' : '후반'} 기록이 반영되었습니다.`});
    } catch (error) {
  ...
  if (!tournament || !match) return <div className="container mx-auto p-8"><Button variant="outline" onClick={handleGoBack} className="mb-4"><ArrowLeft className="mr-2 h-4 w-4" /> 뒤로 가기</Button><p>경기 정보를 찾을 수 없습니다.</p></div>;

  const statCategories: { key: StatCategory, name: string }[] = [
    { key: 'goal', name: '득점' }, { key: 'pass', name: '패스' }, { key: 'dribble', name: '돌파 성공' }, { key: 'defense', name: '수비 성공' },
  ];
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            <Select value={selectedTeamId} onValueChange={setSelectedTeamId}>
              <SelectTrigger className="w-full sm:w-[200px]"><SelectValue placeholder="팀 선택" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={selectedHalf} onValueChange={setSelectedHalf}>
              <SelectTrigger className="w-full sm:w-[120px]"><SelectValue placeholder="시간 선택" /></SelectTrigger>
              <SelectContent>
            ...
            <div className="flex items-center gap-2 sm:ml-auto">
              ...
              <Button onClick={() => window.print()}><Printer className="w-4 h-4 mr-2" />인쇄하기</Button>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {statCategories.map(cat => <TableHead key={cat.key} className="text-center border-l">{cat.name}</TableHead>)}
              </TableRow>
            </TableHeader>
            <TableBody>
              ...
              <TableRow className="bg-muted/50 hover:bg-muted/50 font-bold">
                ...
                {statCategories.map(cat => <TableCell key={`total-${cat.key}`} className="text-center border-l">{tableTotals[cat.key]}</TableCell>)}
              </TableRow>
            </TableBody>
          </Table>
